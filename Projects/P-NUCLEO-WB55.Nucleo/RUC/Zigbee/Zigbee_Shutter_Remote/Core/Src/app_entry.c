...
static void APPE_SysEvtError( SCHI_SystemErrCode_t ErrorCode)
{
  switch(ErrorCode)
  {
    case ERR_ZIGBEE_UNKNOWN_CMD:
       APP_ZB_DBG("** ERR_ZIGBEE : UNKNOWN_CMD \n");
       break;
	   
    default:
       APP_ZB_DBG("** ERR_ZIGBEE : ErroCode=%d \n", ErrorCode);
       break;
  }
  return;
} /* APPE_SysEvtError */
...
/**
  ...
  * @param  GPIO_EXTI_Pin External Interrupt which has been activated
  * @attention same pin value on different GPIO has the same action
  * @retval None
  */
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_EXTI_Pin)
{

  switch (GPIO_EXTI_Pin)
  {
    case BUTTON_SW1_EXTI_LINE:
    UTIL_SEQ_SetTask(1U << CFG_TASK_BUTTON_SW1,CFG_SCH_PRIO_1);
    break;

    case BUTTON_SW2_EXTI_LINE:
    UTIL_SEQ_SetTask(1U << CFG_TASK_BUTTON_SW2,CFG_SCH_PRIO_1);
    break;

    case BUTTON_SW3_EXTI_LINE:
...
static void RxCpltCallback(void)
{
  ...
  if (indexReceiveChar < C_SIZE_CMD_STRING)
  {
    if (aRxBuffer[0] == '\r')
    {
      APP_ZB_DBG("received %s", CommandString);
...
static void UartCmdExecute(void)
{
  ...
  {
    APP_ZB_DBG("NOT RECOGNIZED COMMAND : %s", CommandString);
  }
}
